...
// Enhanced book data - merged from both files, taking seed.js structure as base
const bookData = [
  {
...
// Enhanced feedback data - merged from both files
const queryData = [
  {
    subject: "Request for more programming books",
    message:
      "Could you please add more books on Python and Machine Learning? The current collection is limited.",
    type: "suggestion",
    priority: "medium",
    status: "open",
  },
  {
    ...
    message:
      "Can the library stay open until 10 PM on weekdays? It would be very helpful for working professionals.",
    type: "inquiry",
    priority: "low",
    status: "in_progress",
  },
  {
    ...
    message:
      "It would be great to have a book reservation system for books that are currently borrowed.",
  ...
  },
  {
    ...
    message:
      "The copy of '1984' I borrowed has several torn pages. Please check other copies.",
    type: "complaint",
    priority: "medium",
    status: "resolved",
    response:
      "Thank you for reporting. We have checked and replaced the damaged copies.",
  },
  {
    subject: "Late fee inquiry",
    message:
      "I was charged a late fee but I returned the book on time. Can you please check?",
    type: "complaint",
    priority: "high",
    status: "resolved",
    response:
      "We have reviewed your case and refunded the late fee. Our apologies for the error.",
  },
];

// Notifications data from seed2.js
const notificationsDataFromSeed2 = [
  {
    title: "New Books Added",
    message:
      "We've added 10 new programming books to our collection. Check them out!",
    type: "general",
  },
  {
    title: "Library Maintenance",
    message:
      "The library will be closed for maintenance on Sunday from 9 AM to 2 PM.",
    type: "general",
  },
  {
    title: "Reading Challenge 2024",
    message:
      "Join our annual reading challenge! Read 12 books and win exciting prizes.",
    type: "info",
  },
];
  ...
  try {
    ...
    for (let i = 0; i < queryData.length; i++) {
      try {
        const query = new Query({
          ...
          resolvedAt:
            queryData[i].status === "resolved" ? new Date() : undefined,
        });

        const saved = await query.save();
        queries.push(saved);
        console.log(`✅ Inserted query: ${query.subject}`);
      } catch (error) {
    ...
    for (const borrowData of borrowsData) {
      try {
        const borrow = new Borrow({
          ...
          notes: borrowData.notes || `Borrow record for ${borrowData.status}`,
        });
    ...
    for (const borrow of borrows) {
      ...
      if (borrow.user_id && borrow.book_id) {
        borrowMap[`${borrow.user_id.toString()}_${borrow.book_id.toString()}`] =
          borrow;
      }
    }
    ...
    // Comprehensive notifications for John Doe
    ...
    // Insert notifications from seed.js
    const notifications = [];
